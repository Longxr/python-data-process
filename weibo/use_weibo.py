...
class WeiboClient(object):
    ...
    def get_token(self, code):
        ...
        if self.token:
            return self.token
        url = self.API_URL + 'oauth2/access_token?client_id={0}&client_secret={1}&grant_type=authorization_code&redirect_uri=http://lxrtest.com&code={2}'.format(
            self.client_id,
            self.client_secret,
            code
        )
    ...
    def weibo_share(self):
        ...
        resp = self.fetch('POST', url, {
            ...
            'status': 'API statuses/share test, {0} http://lxrtest.com'.format(datetime.now())
        })
        return resp


client_id = '1781091233'
client_secret = '66642b1b50b94231284c8e9b8ba9d666'
