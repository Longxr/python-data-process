...
if __name__ == '__main__':
    ...
    template_ui_getter_file = open(
        os.path.join(root_dir, 'ui_getter.template'), 'r')
    template_layout_getter_file = open(
        os.path.join(root_dir, 'layout_getter.template'), 'r')
    ...
    parent_widget = parent_widget.strip(';')
    ...
    while 1:
        ...
        out_h = h_tmpl.safe_substitute(
            class_name=name_list[0], var_name=name_list[1], parent_name=parent_widget)
        out_h_file.write(out_h)

        out_cpp = ""
        if "Layout" in name_list[0]:
            out_cpp = layout_getter_tmpl.safe_substitute(
                class_name=name_list[0], var_name=name_list[1], parent_name=parent_widget)
        else:
            out_cpp = ui_getter_tmpl.safe_substitute(
                class_name=name_list[0], var_name=name_list[1], parent_name=parent_widget)
