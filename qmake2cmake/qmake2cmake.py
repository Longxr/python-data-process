...
def _process_vars_in_literal(literal):
    ...
    for src in predefined_replaces.keys():
...
class StatementCondition:
    ...
    def _process_build_type_condition(self):
        ...
        if self.condition.startswith(prefix):
            report_warning(
                "support for build type conditions like CONFIG(release, release|debug) is very poor")
            qmake_build_type = self.condition[len(
                prefix):self.condition.find(",")]
            build_types = {"debug": "Debug",
                           "release": "Release", "msvc": "MSVC"}
            self.condition = "CMAKE_BUILD_TYPE MATCHES " + \
                build_types[qmake_build_type]

    def _process_non_empty_condition(self):
        prefix = "!isEmpty("
        if self.condition.startswith(prefix):
            report_warning(
                "support for !isEmpty condition is not fully correct")
            self.condition = self.condition[len(
                prefix):self.condition.index(")")]
...
class StatementConcatenation:
    ...
    def _process_special_define(self):
        if self.var == "DEFINES":
            qmake_definitions = " ".join(
                ["-D" + define.strip() for define in self.val.split()])
        ...
        if self.var == "QT":
            ...
            for qt_component in self.val.split():
                ...
                try:
                    ...
                    pass
                self.special += "find_package(Qt5" + \
                    qt_component_cmake + " REQUIRED)\n"
                qt_libraries += "Qt5::" + qt_component_cmake + " "
            self.special += StatementConcatenation(
                "LIBS += " + qt_libraries.strip()).cmake_code() + "\n"
...
class StatementFunction:
    def __init__(self, line):
        self.line = line
        self.functions = {"message": self._message,
                          "greaterThan": self._greaterThan, "system": self._system}

    def cmake_code(self):
        left_bracket_pos = self.line.index('(')
        func_name = self.line[:left_bracket_pos].strip()
        right_bracket_pos = self.line.index(')')
        func_args = [arg.strip() for arg in self.line[left_bracket_pos +
                                                      1:right_bracket_pos].split(",")]
...
class QMakeParser:
    ...
    def iter_statements(self):
        for line in self._iter_lines():
            ...
            # todo: better determine this
            elif line.count("}") == 1 and line.count("{") == 1 and "else" in line:
                ...
                report_warning(self._report_on_line(
                    "skipping unknown qmake statement"))

    def _iter_lines(self):
        line = ""
        if "../" in self.input_file_name:
            self.input_file_name = "Z:/CMake/CastControllerCMake/Software/Applications/Screencast/" + \
                self.input_file_name
        with open(self.input_file_name, "r", encoding='utf-8') as qmake_file:
...
class Qmake2CmakeConverter:
    ...
    def convert(self):
        with open(self.cmake_file_name, "w", encoding='utf-8') as cmake_file:
    ...
    def _write_predefined(self, cmake_file):
        ...
        predefined_variables = ["SOURCES", "HEADERS", "RESOURCES",
                                "FORMS", "INCLUDEPATH", "LIBS", "QMAKE_LFLAGS"]
        statements = [StatementComment("#predefined variables")]
        statements.extend([StatementAssignment(var + " = \"\"")
                           for var in predefined_variables])
        statements.append(StatementAssignment("PWD = \"" + os.getcwd() + "\""))

        if self.qt_path is not None:
            statements.append(StatementAssignment(
                "CMAKE_PREFIX_PATH = \"" + self.qt_path + "\""))
...
input_qmake_file_name = "AllProjects.pro"
